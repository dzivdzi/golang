...
func main() {
	// 1
	jb := map[string][]string{"bond_james": {"shaken, not stirred", "martinis", "fast cars"}, "moneypenny_jenny": {`intelligence`, `literature`, `computer science`}, "no_dr": {`cats`, `ice cream`, `sunsets`}}
	// 2
	jb["fleming_ian"] = []string{`steaks`, `cigars`, `espionage`}
	fmt.Println(jb)
	// 3
	delete(jb, "fleming_ian")
	fmt.Println(jb)
	// 4
	ggtb := map[string]int{}
